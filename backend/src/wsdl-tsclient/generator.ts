        ...
        if (options.modelPropertyNaming) {
            switch (options.modelPropertyNaming) {
                case ModelPropertyNaming.camelCase:
                    prop.name = camelcase.default(prop.name);
                    break;
                case ModelPropertyNaming.PascalCase:
                    prop.name = camelcase.default(prop.name, { pascalCase: true });
                    break;
            }
        }
        if (prop.kind === "PRIMITIVE") {
            // e.g. string
            definitionProperties.push(createProperty(prop.name, prop.type, prop.description || '', prop.isArray || false));
        ...
        for (const port of parsedWsdl.ports) {
            ...
            for (const method of port.methods) {
                ...
                portFileMethods.push({
                    ...
                    parameters: [
                        {
                            name: camelcase.default(method.paramName),
