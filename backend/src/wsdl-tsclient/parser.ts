                    ...
                    if (typeof type === "string") {
                        ...
                        if (typeComponents.length >= 2) {
                            definition.properties.push({
                                ...
                                sourceName: propName,
                                description: type,
                                type: ['xs:decimal', 'xs:integer'].includes(typeComponents[1]) && 'number' || 'string',
                                isArray: false,
                            });
                        } else {
                            definition.properties.push({
                                ...
                                sourceName: propName,
                                description: type,
                                type: ['xs:decimal', 'xs:integer'].includes(type) && 'number' || 'string',
                                isArray: false,
                            });
                        }
                    ...
                    for (const [portName, port] of Object.entries(service.ports)) {
                        ...
                        for (const [methodName, method] of Object.entries(port.binding.methods)) {
                            ...
                            if (method.input) {
                                ...
                                const inputMessage = wsdl.definitions.messages[method.input.$name as string];
